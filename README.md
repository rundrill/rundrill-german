...
Your personal **German coach** inside your AI agent — short production-first drills, an honest
...
The coach explains in your native language and moves toward German as your level rises
(comprehensible input, i+1), and drills what German actually turns on — the four cases, noun gender, V2 and verb-final word order, separable verbs, two-way prepositions, adjective endings, Konjunktiv II.

## One plugin, three hosts

The coaching skill (`skills/german-coach/SKILL.md`) and `.mcp.json` are shared; each host reads its
...
- **Claude Code / Desktop** — via the RunDrill languages marketplace:
  ...
  /plugin install rundrill-german@rundrill-lang
  ```
  Then run `/german-coach`.
- **OpenAI Codex** — add the RunDrill languages catalog, then install `rundrill-german`:
  ```
  codex plugin marketplace add rundrill/rundrill-lang
  ```
- **Google Antigravity** — drop this folder into `~/.gemini/config/plugins/rundrill-german/` (global)
  or `<workspace>/.agents/plugins/rundrill-german/` (workspace-scoped).
...
Required attribution when sharing: *“RunDrill German coach — © RunDrill, CC BY-NC-ND 4.0,
https://rundrill.com”.*
